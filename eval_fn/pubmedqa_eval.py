...
def pubmedqa_metrics(args):
    ...
    if args.style != 'first':
        ...
        for idx in tqdm(index_list):
            sample = dataset[idx]
            prediction, label = mmlu_judge(sample['answer'], sample['generation'])
            if label == 1: 
        ...
        for idx in tqdm(range(len(dataset))):
            ...
            if not sample['question'] in question_set:
                prediction, label = pubmedqa_judge(sample['answer'], sample['generation'])
                if label == 1:
                    statistics["correct"] += 1
                else:
                    statistics["incorrect"] += 1
...
def pubmedqa_judge(answer, prediction):
    answer = answer.split('\n#### ')[-1]
